unique.multiPhylo <- function (x, incomparables = FALSE, use.edge.length = FALSE, use.tip.label = TRUE, ...) {
	...
	for (i in 2:n) {
		...
		for (s in 1:length(keep)) { # this line changed...
			if (all.equal(x[[keep[s]]], x[[i]], use.edge.length = use.edge.length, use.tip.label = use.tip.label, ...)) ### and this line
                {
                already.seen <- TRUE
                message(paste('breaking out of tree', i, 'which is not unique'))
                tree.vector[i] <- s
				break
            }
        }
